...
class Encyclopedia(callbacks.Plugin):
    ...
    def search_factoid(self, factoid, channel):
        ...
        if not ret:
            return "None found"
        return 'Found: %s' % ', '.join(sorted(list(ret.keys()), lambda x, y: cmp(ret[x], ret[y]))[:10])
    ...
    def sync(self, irc, msg, args, channel):
        ...
        def download_database(location, dpath):
            ...
            fd = urllib.request.urlopen(location)
            fd2 = open(tmp_db, 'w')
    ...
    def lookup(self, irc, msg, args, author):
        ...
        if not ret:
            ...
            rmsg = 'Authored Found: %s' % ', '.join(sorted(list(ret.keys()), lambda x, y: cmp(ret[x], ret[y]))[:10])
        if not log_ret:
            log_rmsg = "Edited: None found"
        else:
            log_rmsg = 'Edited Found: %s' % ', '.join(sorted(list(log_ret.keys()), lambda x, y: cmp(log_ret[x], log_ret[y]))[:10])
        irc.reply(rmsg)
        irc.reply(log_rmsg)
